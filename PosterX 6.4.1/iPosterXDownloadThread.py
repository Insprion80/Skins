...
PROVIDER_CONNECT_TIMEOUT = 0.5             # faster connect cutoff (1.5)
PROVIDER_READ_TIMEOUT    = 0.5             # don't wait long for slow providers (3.0)
DOWNLOAD_CONNECT_TIMEOUT = 0.5             # poster image fetch must connect fast (1.5)
DOWNLOAD_READ_TIMEOUT    = 1.0             # allow a bit more for image download (5.0)
...
def log(msg, level="INFO"):
    """
    Compact logger with autotruncate. Skip provider success messages to reduce noise.
    """
    try:
...
class iPosterXDownloadThread(threading.Thread):
    ...
    def savePoster(self, url, callback, cleaned_title=None, source=None):
        ...
        source (optional) indicates provider, e.g. 'tmdb', 'imdb', etc.
        """
        log("savePoster start (attempt)", "DEBUG")
        try:
            ...
            if ok:
                # update index (filename derived from cleaned_title + ".jpg")
                try:
                    if cleaned_title:
    ...
    # ---------- Providers (each is small, fast, returns (ok, logmsg, poster_url or None)) ----------
    ...
    def provider_imdb(self, title, shortdesc, fulldesc):
        """
        Uses public IMDb suggestion API (fast, no key):
        https://v2.sg.media-imdb.com/suggestion/<first_letter>/<encoded_title>.json
        """
        try:
            if not title:
            ...
            for it in data.get('d', []):
                ...
                if img and isinstance(img, (list, tuple)) and img[0].startswith('http'):
                    poster = img[0]
                    return True, "[SUCCESS imdb] %s => %s" % (title, poster), poster
    ...
    def provider_tvdb(self, title, shortdesc, fulldesc):
        """
        TVDB v4 needs login token; legacy endpoints are unreliable.
        Keep lightweight: attempt quick HTML search page and try to grab an image.
        If fails, return SKIP quickly (does not block).
        """
        try:
            if not title:
                return False, "[SKIP tvdb] empty title", None
            # very light best-effort: search page
            q = urlquote(title)
            ...
            htmltxt = resp.text
            # naive look for first poster-style image
            ...
            if m:
                ...
                return True, "[SUCCESS tvdb] %s => %s" % (title, poster), poster
    ...
    def provider_fanart(self, title, shortdesc, fulldesc):
        """
        fanart.tv generally needs an ID. Without ID, try a very quick search page fallback.
        """
        try:
            if not title:
                return False, "[SKIP fanart] empty title", None
            # quick web search to fanart.tv (lightweight)
            ...
            htmltxt = resp.text
            # simple image url pattern to fanart
            m = re.search(r'https?://assets\.fanart\.tv/[^"\']+\.jpg', htmltxt)
            if m:
                poster = m.group(0)
                return True, "[SUCCESS fanart] %s => %s" % (title, poster), poster
    ...
    def provider_google(self, title, shortdesc, fulldesc):
        """
        Last-resort HTML scrape of Bing Images.
        We fish the first "murl" original image URL from the page script.
        """
        try:
            if not title:
            ...
            if m:
                poster = m.group(1)
                return True, "[SUCCESS web] %s => %s" % (title, poster), poster
    ...
    def parallel_search_and_save(self, dwn_poster, title, shortdesc, fulldesc):
        """
        Launch small threads for each provider; first one to yield a valid poster wins.
        Others will be ignored once a winner is set.
        Returns True if a poster download/save was started or already exists.
        """
        stop_flag = {"stop": False}
        lock = threading.Lock()
        winner = {"url": None, "log": [], "provider": None}
        ...
        # Log only non-success lines to reduce noise
        for l in winner["log"]:
    ...
    # ---------- Queue worker ----------
    def run(self):
        log("PosterDB thread initialized")
        while True:
            canal = pdb.get()
            try:
                raw_event = canal[5] if canal and len(canal) > 5 else None
                if not raw_event:
                    log("None type detected - poster not found")
                    try:
                        pdb.task_done()
                    except Exception:
                        pass
                ...
                if dwn_poster_v1 and os.path.exists(dwn_poster_v1):
                    ...
                    # Before searching remote providers, check poster index for mapping (maybe downloaded earlier)
                    try:
                        ...
                        if indexed and os.path.exists(indexed):
                            ...
                            log("[TRY v1] {}".format(title_v1))
                            got = self.parallel_search_and_save(dwn_poster_v1, title_v1, shortdesc, fulldesc)
                    except Exception as e:
                        log("[ERROR index-check v1] %s" % e, "ERROR")

                # --- If v1 failed → fallback to v2 ---
                if not got:
                    try:
                        from .iConverlibr import convtext_fallback
                    except Exception:
                        convtext_fallback = None
                    if convtext_fallback:
                        title_v2 = convtext_fallback(raw_event)
                    else:
                        title_v2 = None
                    dwn_poster_v2 = os.path.join(path_folder, title_v2 + ".jpg") if title_v2 else None

                    if dwn_poster_v2 and os.path.exists(dwn_poster_v2):
                        try:
                            os.utime(dwn_poster_v2, (time.time(), time.time()))
                        except Exception:
                            pass
                    elif title_v2:
                        # check index for v2 mapping before searching
                        try:
                            indexed2 = poster_index_get_fullpath(title_v2)
                            if indexed2 and os.path.exists(indexed2):
                                try:
                                    os.utime(indexed2, (time.time(), time.time()))
                                except Exception:
                                    pass
                            else:
                                log("[FALLBACK v2] {}".format(title_v2))
                                self.parallel_search_and_save(dwn_poster_v2, title_v2, shortdesc, fulldesc)
                        except Exception as e:
                            log("[ERROR index-check v2] %s" % e, "ERROR")
                ...
                try:
                    with open("/tmp/iPosterDB.log", "a") as w:
                        w.write("%s\n" % e)
                except Exception:
                    pass
                try:
