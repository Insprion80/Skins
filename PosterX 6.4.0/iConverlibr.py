...
Added:
 - PosterIndex class: small JSON-backed index stored in poster folder
   (maps cleaned_title -> [timestamp, source]) with LRU cap.
 - Simple module-level helpers:
     init_poster_index(path_folder, max_entries)
     poster_index_add(cleaned_title, source)
     poster_index_get_fullpath(cleaned_title)
 - NOTE: index stores filename as cleaned_title + ".jpg" implicitly;
         we store only timestamp and source in value array to save space.
"""
...
class PosterIndex(object):
    """
    Keep a small JSON index mapping cleaned_title -> [timestamp, source].
    ...
    Methods:
      ...
      - add(clean_title, source)
      - get(clean_title) -> (timestamp, source) or None
      - get_fullpath(clean_title) -> filename path if poster present
      - save(), load()
    Notes:
      - We assume file naming convention: cleaned_title + ".jpg"
      - Value is a list: [timestamp, source_string]
    """
    ...
    def __init__(self, path_folder=None, max_entries=5000):
        ...
        self.map = {}        # cleaned_title -> [timestamp, source]
        self.order = []      # oldest -> newest cleaned_title
        self._loaded = False
        self.load()

    @classmethod
    def get_instance(cls, path_folder=None, max_entries=50000):
    ...
    def add(self, cleaned_title, source="remote"):
        """
        Add mapping cleaned_title -> [timestamp, source].
    ...
    def get(self, cleaned_title):
        """
        Return [timestamp, source] or None.
        """
        if not cleaned_title:
            return None
        key = cleaned_title.strip()
    ...
    def get_fullpath(self, cleaned_title):
        """
        Return full poster path if file exists on disk, else None.
        Filename derived as cleaned_title + ".jpg"
        """
        if not cleaned_title:
            return None
...
def convtext_fallback(text=''):
    """
    Simple emergency fallback cleaner if convtext() fails.
    Just strips Arabic/English episode numbers and trims whitespace.
    Ensures downloader always has something usable.
    """
    if not text:
        return ""
    try:
