...
"""
iConverlibr.py - XDREAMY SKIN V8.21
...
_S_NUM_RE = re.compile(r"(?<!\w)S\s*(\d{1,3})\b", re.I | re.U)
_S_ROMAN_RE = re.compile(r"(?<!\w)S\s+(" + _ROMAN_NUM + r")\b", re.I | re.U)
...
def prepare_search_title(raw):
    ...
    # S2 / S II is an identity-adjacent context block. Keep numeric and
    # Roman forms separate so ordinary words such as "Six" or "si" are not
    # interpreted as season markers.
    if season is None:
        m = _S_NUM_RE.search(text)
        if m:
            season = int(m.group(1))
            add("SEASON", m.group(1))
            text = _remove_span(text, m)
    ...
    # Episode marker.  Before removing it, recover a season that is immediately
    # attached to the episode block. This covers common EPG forms such as:
    #   "Private Eyes II ep.17"
    #   "Masha e Orso 7 EP.5"
    #   "Heartland 4 - Fiocco rosa ep.18"
    # The explicit episode marker makes these season inferences safe; without
    # it, the same numbers remain ordinary title text.
    em = _EP_MARKER_RE.search(text)
    if em and season is None:
        prefix_for_season = text[:em.start()].rstrip()

        # Simple trailing numeric/Roman season: "Title 7 EP.5" / "Title II ep.17"
        sm = re.search(r"(?:^|\s)(?P<s>\d{1,2}|" + _ROMAN_NUM + r")\s*$", prefix_for_season, re.I | re.U)
        if sm:
            value = sm.group("s")
            if not (value.isdigit() and re.fullmatch(r"(?:19|20)\d{2}", value)):
                season = _number_value(value)
                add("SEASON", value)
                prefix_for_season = prefix_for_season[:sm.start()].rstrip(" -,:;")

        # Common form: "Title 4 - Episode Name ep.18". Restrict the season
        # to 1-2 digits and require the explicit episode marker above.
        if season is None:
            sm = re.search(r"(?:^|\s)(?P<s>\d{1,2})\s+-\s+.+$", prefix_for_season, re.I | re.U)
            if sm:
                season = int(sm.group("s"))
                add("SEASON", sm.group("s"))
                # Remove only the season number; preserve the dash/episode
                # title block so the normal episode-title parser can use it.
                prefix_for_season = re.sub(
                    r"(\s)" + re.escape(sm.group("s")) + r"(?=\s+-\s+)",
                    r"\1",
                    prefix_for_season,
                    count=1,
                ).strip()

        # Another common feed form: "... 2: odc.5". The explicit episode
        # marker is already known, so a final 1-2 digit number before a colon
        # is treated as season context rather than title punctuation.
        if season is None:
            sm = re.search(r"(?:^|\s)(?P<s>\d{1,2})\s*:\s*[^:]+$", prefix_for_season, re.I | re.U)
            if sm:
                season = int(sm.group("s"))
                add("SEASON", sm.group("s"))
                # Keep the title block before the colon; remove only the
                # numeric season marker.
                prefix_for_season = re.sub(
                    r"(\s)" + re.escape(sm.group("s")) + r"(?=\s*:)",
                    r"\1",
                    prefix_for_season,
                    count=1,
                ).strip()
        ...
        if season is not None:
            text = prefix_for_season + " " + text[em.start():].lstrip()
            em = _EP_MARKER_RE.search(text)
    ...
    if em:
        ...
        if pm:
            ...
            if sm:
                ...
                sm = re.search(r"(?:^|\s)(\d{1,2})\s*$", before, re.I | re.U)
                if sm:
    ...
    # Do not infer a secondary title from normal punctuation such as a colon.
    # A colon is part of many legitimate program names. Secondary-title data
    # is only populated when an upstream parser explicitly identifies it.
    secondary_title = ""
