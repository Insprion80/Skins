...
# FAST Enhanced: minimal year extraction for better poster accuracy while keeping original speed
...
from .iConverlibr import (
    convtext, cutName, REGEX, poster_index_get_fullpath, init_poster_index,
    get_year_from_description
)
...
class PosterAutoDB(iPosterXDownloadThread):
    ...
    def run(self):
        self.logAutoDB("[AutoDB] *** Initialized (FAST mode with minimal year enhancement) ***")
        while True:
            # default behavior: every 2 hours
            time.sleep(600)  # 7200 - Start every 2 hours
            self.logAutoDB("[AutoDB] *** Running ***")
            self.pstcanal = None
            # AUTO ADD NEW FILES - 1440 (24 hours ahead)
            for service in apdb.values():
                try:
                    # lookup events for the next 1440 minutes (24h)
                    ...
                    for evt in events:
                        ...
                        if evt[1] is None or evt[4] is None or evt[5] is None or evt[6] is None:
                            ...
                            if not os.path.exists(dwn_poster):
                                # FAST: pass description fields to parallel search (minimal processing)
                                shortdesc = canal[4] if len(canal) > 4 else ""
                                fulldesc = canal[3] if len(canal) > 3 else ""
                                ok = self.parallel_search_and_save(dwn_poster, self.pstcanal, shortdesc, fulldesc)
                                if ok:
                                    newfd += 1
                                else:
...
# ------------------------------------------------------------------
# FAST Enhanced Renderer class with minimal year enhancement
# ------------------------------------------------------------------
class iPosterX(Renderer):
    def __init__(self):
        ...
        if not self.adsl:
            ...
            print("Connessione rilevata (FAST mode).")
        self.nxts = 0
    ...
    def changed(self, what):
        ...
        try:
            ...
            # FAST: minimal year extraction for better accuracy (only log if found)
            shortdesc = self.canal[4] if len(self.canal) > 4 else ""
            fulldesc = self.canal[3] if len(self.canal) > 3 else ""
            
            # Only extract year if descriptions are reasonably short (for performance)
            if (shortdesc or fulldesc) and len(str(shortdesc) + str(fulldesc)) < 200:
                year = get_year_from_description(shortdesc, fulldesc)
                if year:
                    self.logPoster("Found year: {} for better search accuracy".format(year))

            # Use convtext (unchanged) to derive cleaned search title
            self.pstcanal = convtext(self.canal[5])

            # Check index for immediate mapping to poster file (FAST lookup)
            mapped_path = None
            try:
                if self.pstcanal:
