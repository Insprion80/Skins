...
Added:
 ...
 - MINIMAL year extraction for better poster accuracy (performance focused)
"""
...
REGEX = compile(
    ...
    r'[хмтдХМТД]/[фф]\s|'                  # Russian markers
    r'\s[сС](?:езон|ерий|-н|-я)\s.*|'      # Season/Episode in Russian
    r'\s\d{1,3}\s[чяЧЯ]\.?\s.*|'           # " 12 ч"
    r'\.\s\d{1,3}\s[чяЧЯ]\.?\s.*|'         # ". 12 ч"
    r'\s[чяЧЯ]\.?\s\d{1,3}.*|'             # "ч 12"
    r'\d{1,3}-(?:я|й)\s?с-н.*',            # Russian suffix
    re.DOTALL
)

# -------------------------------------------------------------------
# FAST Year Extraction (minimal processing)
# -------------------------------------------------------------------
def get_year_from_description(shortdesc="", fulldesc=""):
    """
    FAST year extraction - only check obvious patterns.
    Returns year string or None. Optimized for speed.
    """
    # Combine descriptions quickly
    text = ""
    if shortdesc:
        text = str(shortdesc)
    if fulldesc and len(text) < 100:  # Don't process very long descriptions
        text = text + " " + str(fulldesc)
    
    if len(text) < 4:  # Need at least 4 chars for a year
        return None
    
    # Quick patterns only - prioritize speed
    # Look for (YYYY) first - most reliable
    ...
    years = re.findall(r'\b(19[6-9]\d|20[0-3]\d)\b', text)
    if years:
        return years[0]  # Return first found
...
if __name__ == "__main__":
    ...
    print("=== Basic Cleaning Tests ===")
    for s in samples:
        print("RAW: %s  -->  CLEAN: %s  DISPLAY: %s" % (s, convtext(s), convtext_display(s)))
    
    print("\n=== Fast Year Extraction Tests ===")
    test_descriptions = [
        ("Action movie from 2020", "Great movie!"),
        ("(2019) Science fiction", "A film by Denis Villeneuve"),
        ("Comedy", "Released in 2018, starring..."),
        ("", "Movie from 1999"),
    ]
    
    for short, full in test_descriptions:
        year = get_year_from_description(short, full)
        print("SHORT: '{}' | FULL: '{}' => YEAR: {}".format(short, full, year))