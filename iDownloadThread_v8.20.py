...
"""
iDownloadThread.py - XDREAMY V8.20
    ...
    for query_index, query in enumerate(queries):
        ...
        for endpoint_index, media_type in enumerate(endpoints):
            ...
            for rank, item in enumerate(results[:8], 1):
                ...
                if quick > 0:
                    ranked.append((quick, rank, item))
            # Keep TMDB's relevance order as the primary ordering. The quick
            # score is only a gate that prevents wasting detail calls on clearly
            # unrelated results.
            ranked.sort(key=lambda x: x[1])

            # Preserve TMDB's first result when no cheap lexical clue exists;
            # localized titles can legitimately differ completely.
            if not ranked and results:
                ranked = [(0, 1, results[0])]

            first_valid = None
            for quick, rank, item in ranked[:2]:
